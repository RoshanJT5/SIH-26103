...
@router.get(
    ...
    description="Returns datasets newest first for snapshot selectors and audit views.",
)
def list_datasets(db: Session = Depends(get_db)) -> list[DatasetListItem]:
...
def get_dataset_quality(
    ...
    summary_rows = db.execute(
        select(
            IngestionIssue.severity,
            IngestionIssue.issue_code,
            func.count(IngestionIssue.id),
        )
    ...
    return DatasetQualityResponse(
        ...
        limit=limit,
        summary=[
            QualitySummary(severity=row[0], issue_code=row[1], count=row[2])
