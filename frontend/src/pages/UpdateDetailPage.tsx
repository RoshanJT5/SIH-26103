...
export default function UpdateDetailPage() {
  ...
  if (error) return <section className="section"><div className="wrap"><div className="alert alert-error" role="alert">Unable to load update. {error}</div><Link className="link" to="/updates" style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><ArrowLeft size={13} />Back to Updates</Link></div></section>;
  if (!item) return null;
  return (
    <section className="section">
      <div className="wrap">
        <p><Link className="link" to="/updates" style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><ArrowLeft size={13} />Back to Updates</Link></p>
        ...
        {item.related_dataset_id && <p style={{ marginTop: 12 }}><Link className="link" to="/documents" style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>View related dataset <ArrowRight size={13} /></Link></p>}
      </div>
    </section>
  );
}
