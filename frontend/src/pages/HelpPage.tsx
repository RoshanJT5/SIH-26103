...
export default function HelpPage() {
  ...
  return (
    <section className="section faq" aria-labelledby="t">
      <div className="wrap">
        <div className="section-head"><p className="section-eyebrow">Help &amp; grievance</p><h2 id="t">Help, FAQs and support</h2><p>Service status: <code>GET /health</code> → {health ? `${health.status} / DB ${health.database} / assistant ${health.assistant_enabled ? "enabled" : "disabled"}` : "checking…"} · API: {API_URL}</p></div>
        ...
        <div className="card-grid" style={{ marginTop: 16 }}>
          <article className="card"><h3>Contact support</h3><p>Access, data questions or accessibility assistance. Backend docs: <a className="link" href="http://127.0.0.1:8000/docs" target="_blank" rel="noreferrer">/docs</a>.</p></article>
          <article className="card"><h3>Keyboard help</h3><p>Tab to move · Enter to open · Esc to close dialogs. Text controls (A- / A / A+) are in the top bar.</p><button className="btn btn-secondary btn-sm" type="button" onClick={() => document.documentElement.classList.toggle("high-contrast")}>Toggle high contrast</button></article>
          <article className="card"><h3>Authentication</h3><p>Admin upload uses <code>POST /auth/login</code> for a short-lived bearer token. Tokens are held in memory for this prototype.</p></article>
        </div>
      </div>
    </section>
  );
