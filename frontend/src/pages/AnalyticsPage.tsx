...
import { Band, bandColors, formatNumber, formatPercent, getJson, clearApiCache, type Group, type Project } from "../api";
...
export default function AnalyticsPage() {
  ...
  const loadData = (force = false) => {
    ...
    Promise.all([
      getJson<Group[]>("/analytics/sectors", { forceRefresh: force }),
      getJson<Group[]>("/analytics/ministries", { forceRefresh: force }),
      getJson<any>("/analytics/cost-drivers", { forceRefresh: force }).catch(() => null),
      getJson<any>("/analytics/geography", { forceRefresh: force }).catch(() => null),
    ])
      .then(([s, m, d, g]) => {
        setSectors(s);
        setMinistries(m);
