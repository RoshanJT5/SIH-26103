...
function delta(value: number | null | undefined, change: number | null | undefined, fmt: (v: number) => string) {
  ...
  return (
    <span style={{ color, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 3 }} title={label}>
      ...
      <div className="wrap">
        <div className="section-head">
          ...
          <h2 id="t" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <GitCompare size={24} color="var(--navy-700)" /> Monitoring Changes
          </h2>
          <p>What changed since the previous monitoring cycle. Matched by stable project identity — not by row position.</p>
        </div>
                ...
                <div key={String(title)} className="panel">
                  ...
                  <div className="table-wrap"><table className="gov-table">
                    ...
                    <tbody>{(items as SnapshotComparison["new_high_risk_projects"]).length === 0 ? <tr><td colSpan={4}>None in this comparison.</td></tr> : (items as SnapshotComparison["new_high_risk_projects"]).map((p) => (
                      <tr key={p.project_id}>
                        <td><strong>{p.project_name}</strong><span className="row-code">{p.project_code} · {p.sector}</span></td>
                        <td>{p.previous_band ?? "—"} <ArrowRight size={12} style={{ verticalAlign: "middle" }} /> {p.current_band ?? "—"}</td>
                        <td>
                          {p.score_change === null ? "—" : `${p.score_change > 0 ? "+" : ""}${formatNumber(p.score_change, 2)}`}
                          {p.score_change !== null && (
                            <span style={{ color: p.score_change > 0 ? "#B42318" : "#16803C", marginLeft: 4, display: "inline-flex", alignItems: "center" }}>
                              {p.score_change > 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                            </span>
                          )}
                        </td>
                        <td>
